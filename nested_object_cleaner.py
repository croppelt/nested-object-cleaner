...
import json
import os
from typing import Any, Dict, Iterable, Optional, OrderedDict
...
def prune_obj(obj: Any, on_keys: Iterable[Any], for_values: Iterable[Any]):
    ...
    if isinstance(obj, dict):
        ...
        for k, v in obj.items():
            if k in on_keys and v in for_values:
                ...
                if v:
                    pruned_v = prune_obj(obj=v, on_keys=on_keys, for_values=for_values)
                    if pruned_v:
                        pruned[k] = pruned_v
                else:
        ...
        for element in obj:
            if element:
                pruned_e = prune_obj(obj=element, on_keys=on_keys, for_values=for_values)
                if pruned_e:
...
def clean_obj(obj: Any, search_keys: Iterable[Any], clean_keys: Iterable[Any]) -> Any:
...
if __name__ == "__main__":
    ...
    cleaned = clean_obj(
        obj=nested_obj,
        search_keys=["id", "source", "elementId"],
        clean_keys=["id"],
    )
    write_dict_to_json(
        dict_=cleaned,
